...
#define ABC_PRTSP(a, t, T) (Abc_Print(1, "%s =", (a)), Abc_Print(1, "%9.5f sec (%6.5f %%)\n", 1.0 * ((double)(t)) / ((double)CLOCKS_PER_SEC), ((double)(T)) ? 100.0 * ((double)(t)) / ((double)(T)) : 0.0))
...
static inline int Abc_Base2LogW(word n)
{
    int r;
    if (n < 2)
        return (int)n;
...
static inline int Abc_Base10LogW(word n)
{
    int r;
    if (n < 2)
        return (int)n;
...
static inline int Abc_Base16LogW(word n)
{
    int r;
    if (n < 2)
        return (int)n;
