...
Pdr_Man_t *Pdr_ManStart(Aig_Man_t *pAig, Pdr_Par_t *pPars, Vec_Int_t *vPrioInit)
{
    ...
    p->vPredicateStatus = Vec_IntStart(Aig_ManRegNum(pAig));
    ...
    {
        Pdr_ManReadRelations(pPars->pRelFileName, pAig, p);
    }
...
void Pdr_ManStop(Pdr_Man_t *p)
{
    ...
    if (p->pPars->fVerbose)
    {
        ...
        ABC_PRTSP("Generalize ", p->tGeneral, p->tTotal);
        ABC_PRTSP("Push clause", p->tPush, p->tTotal);
        ABC_PRTSP("Ternary sim", p->tTsim, p->tTotal);
